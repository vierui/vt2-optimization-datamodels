...
def run_pipeline(cfg):
    ...
    sets_dict, cv_mae, gain_series, bo_trace = build_feature_sets(
        manual_feats,
        rich_tr,
    ...
    # train ALL sets on the *rich* DataFrame so they all line up
    sets_train = {lbl: rich_tr for lbl in sets_dict}
    sets_test  = {lbl: rich_te for lbl in sets_dict}
    ...
    # Bayes search trace plot
    k_vals, mae_vals = zip(*bo_trace)
    plt.figure(figsize=(6,4))
    plt.plot(k_vals, mae_vals, marker="o")
    best_idx = np.argmin(mae_vals)
    plt.axvline(k_vals[best_idx], ls="--", color="k",
                label=f"best k={k_vals[best_idx]}")
    plt.xlabel("Subset size k")
    plt.ylabel("CV MAE")
    plt.title("Bayes search of k")
    plt.legend()
    plt.tight_layout()
    plt.savefig(out_rep / "bayesopt_k_curve.png")
    plt.close()
    ...
    summary = {
        ...
        "bayesopt_trace": {str(k): float(m) for k, m in bo_trace},
        "bayesopt_best_k": int(k_vals[best_idx])
    }
