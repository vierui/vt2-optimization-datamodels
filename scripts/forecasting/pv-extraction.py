...
args = {
    ...
    'azim': 180,
    'format': 'csv',
    'raw': 'true'  # Add raw parameter to get weather data alongside generation data
}
...
file_name = 'pv_with_weather_data.csv'
...
print(f"Preparing to fetch wind data with weather variables from {original_date_from_str} to {original_date_to_str} in chunks constrained by a 1-year period.")
...
if all_chunks_successful:
    if data_written:
        print(f"Wind data with weather variables successfully saved to {file_path}")
    else:
